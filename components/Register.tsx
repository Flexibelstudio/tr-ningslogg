    ...
    useEffect(() => {
        if (selectedOrgId) {
            const fetchLocations = async () => {
                ...
                try {
                    let locs = await firebaseService.getCollection(selectedOrgId, 'locations');
                    
                    if (!locs || locs.length === 0) {
                        console.warn(`Online fetch for locations in org ${selectedOrgId} was empty or failed, falling back to mock data.`);
                        const mockOrgData = dataService.getOrgData(selectedOrgId);
                        locs = mockOrgData?.locations || [];
                    }
