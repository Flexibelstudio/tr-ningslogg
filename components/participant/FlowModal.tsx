...
const FlowItemCard: React.FC<FlowItemCardProps> = ({ item, index, currentUserId, allParticipants, onToggleReaction, onAddComment, onDeleteComment }) => {
...
export const FlowModal: React.FC<FlowModalProps> = ({ isOpen, onClose, ...data }) => {
    const { flowItemsToShow, totalItemCount } = useMemo(() => {
        ...
        data.connections.forEach(conn => {
            if (conn.status === 'accepted') {
                if (conn.requesterId === data.currentUserId) friendIds.add(conn.receiverId);
                if (conn.receiverId === data.currentUserId) friendIds.add(conn.requesterId);
            }
        });

        const allowedParticipantIds = new Set([data.currentUserId, ...friendIds]);
        ...
        return {
            flowItemsToShow: finalItems.slice(0, 30),
                    ...
                    <>
                        ...
                        {totalItemCount > 30 && (
                            <div className="text-center text-base text-gray-500 pt-4">
                                Visar de 30 senaste händelserna.
                            </div>
                        )}
                    </>
                ) : (
                    <div className="text-center py-10">
                        <p className="text-xl text-gray-500">Ditt flöde är tomt.</p>
