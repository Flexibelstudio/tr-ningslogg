...
const getIconForHeader = (headerText: string): JSX.Element | null => {
  ...
  if (lowerHeaderText.includes("tips") || lowerHeaderText.includes("motivation")) return <span className="mr-2 text-xl" role="img" aria-label="Tips">💡</span>;
  if (lowerHeaderText.includes("särskilda råd")) return <span className="mr-2 text-xl" role="img" aria-label="Särskilda råd">ℹ️</span>;
  if (lowerHeaderText.includes("avslutning")) return <span className="mr-2 text-xl" role="img" aria-label="Avslutning">🎉</span>;
...
const renderFeedbackContent = (feedback: string | null): JSX.Element[] | null => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // More specific italic regex to avoid issues with single asterisks in other contexts
...
export const AIProgressFeedbackModal: React.FC<AIProgressFeedbackModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Tips från Coachen" size="xl">
      <div className="space-y-4 min-h-[200px] max-h-[70vh] flex flex-col">
        ...
        {aiFeedback && !isLoading && !error && (
          <div className="overflow-y-auto flex-grow p-1 pr-2">
            {/* The title "Din Feedback:" is removed as the modal itself is titled "Tips från Coachen" */}
