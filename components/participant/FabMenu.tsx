...
export const FabMenu: React.FC<FabMenuProps> = ({ isOpen, onToggle, onClose, workouts, currentParticipantId, onSelectWorkoutCategory, onOpenLogGeneralActivityModal, membership, onOpenUpgradeModal, onOpenBookingModal, integrationSettings, onOpenQrScanner, workoutCategories }) => {
  ...
  const allMenuItems = useMemo(() => {
    ...
    if (integrationSettings.isBookingEnabled) {
      actions.push({
          ...
          isRestricted: false, // Booking is never restricted from FAB, logic is handled in BookingView
      });
      actions.push({
        key: 'check-in',
        label: 'Checka in (QR)',
        ...
        actions.push({
            ...
            onClick: () => {
                if (isRestricted) {
                    ...
                    onSelectWorkoutCategory(category.value);
                }
                onClose();
            },
    ...
    return actions;
}, [workouts, currentParticipantId, membership, onOpenUpgradeModal, onSelectWorkoutCategory, onClose, onOpenLogGeneralActivityModal, integrationSettings, onOpenBookingModal, onOpenQrScanner, workoutCategories]);
