...
export const FabMenu: React.FC<FabMenuProps> = ({
  ...
  return (
    <>
      ...
      <div 
        aria-hidden="true"
        className={`fixed inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100 z-30' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
      ></div>

      <div ref={menuRef} className={`fixed bottom-6 right-6 z-40 flex flex-col items-end ${!isOpen ? 'pointer-events-none' : ''}`}>
        {/* Animated Menu Items Container */}
        <div className={`flex flex-col items-end space-y-3 mb-4 transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
            {displayableCategories.map((categoryOption) => (
              <Button
                ...
                size="md"
                className="shadow-2xl w-auto min-w-[200px] justify-start"
        ...
        <button
          ...
          className={`w-20 h-20 rounded-full flex items-center justify-center text-white shadow-xl transition-all duration-300 ease-in-out transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flexibel-orange pointer-events-auto ${
            isOpen
