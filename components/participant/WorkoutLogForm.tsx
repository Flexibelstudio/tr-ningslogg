...
import { Workout, WorkoutLog, WorkoutExerciseLog, SetDetail, Exercise, WorkoutBlock, ParticipantGoalData } from '../../types';
...
export const WorkoutLogForm: React.FC<WorkoutLogFormProps> = ({
    ...
    const fetchAiSuggestion = async () => {
        ...
        if (matchingPrevEx) {
            ...
            if (prevEntry?.loggedSets?.length > 0) {
                ...
                prevEntry.loggedSets.forEach((set, i) => {
                    previousPerformanceString += `        - Set ${i+1}: ${set.reps} reps @ ${set.weight ?? 0} kg (${set.isCompleted ? 'Slutfört' : 'Ej slutfört'})\n`;
                });
            }
        }
      });
      ...
      const prompt = `
        ...
        3.  **Analysera föregående prestation NOGGRANT**:
            *   Titta på både **objektiv data** (set, reps, vikt) och **subjektiv feedback** (kommentar, känsla).
                ...
                if (previousExerciseDataForRefId) {
                    ...
                    if (previousLogEntryForThisExercise && previousLogEntryForThisExercise.loggedSets && previousLogEntryForThisExercise.loggedSets.length > 0) {
                        ...
                        if (!isQuickLogEnabledForBlock) {
                            setsToInit = previousLogEntryForThisExercise.loggedSets.map(() => ({
                                id: crypto.randomUUID(), reps: '', weight: '', isCompleted: false
                            }));
  ...
  const handleAddSupersetRound = (blockId: string, supersetIdentifier: string) => {
    setLogEntries(prev => {
        ...
        exercisesInSuperset.forEach(ex => {
            ...
            const lastSetWeight = (isNewSession || sets.length === 0) ? '' : (sets[sets.length - 1].weight || '');
            newEntries.set(ex.id, [...sets, { id: crypto.randomUUID(), reps: '', weight: lastSetWeight, isCompleted: false }]);
        });
        return newEntries;
    });
  };
  ...
  const handleApplyQuickLogOrAddOne = (blockId: string, supersetIdentifier: string) => {
    ...
    if (roundsStr.trim() !== '' && !isNaN(rounds) && rounds > 0) {
      setLogEntries(prev => {
        ...
        const firstRoundTemplate = new Map<string, { reps: string | number, weight?: string | number }>();

        exercisesInSuperset.forEach(ex => {
            const existingSets = prev.get(ex.id) || [];
            if (existingSets.length > 0) {
                const firstSet = existingSets[0];
                // Use the first set's data if it's filled, otherwise it will be empty strings.
                firstRoundTemplate.set(ex.id, { reps: firstSet.reps, weight: firstSet.weight });
            }
        });

        exercisesInSuperset.forEach(ex => {
            const template = firstRoundTemplate.get(ex.id);
            // Use template values if they exist and are not empty, otherwise use defaults.
            const repsForNewSets = (template?.reps && String(template.reps).trim() !== '') ? template.reps : '1';
            const weightForNewSets = (template?.weight !== undefined && String(template.weight).trim() !== '') ? template.weight : '0';

            const newSets: SetDetail[] = Array.from({ length: rounds }, () => ({
                id: crypto.randomUUID(),
                reps: repsForNewSets,
                weight: weightForNewSets,
                isCompleted: true 
            }));
  ...
  const actuallySubmitLog = () => {
    ...
    logEntries.forEach((sets, exerciseId) => {
      ...
      for (const currentSet of completedSets) {
        ...
        let weightValue = weightValueString === '' ? undefined : Number(weightValueString);
        const exerciseDetails = exercisesToLog.find(ex => ex.id === exerciseId);
        if (exerciseDetails?.isBodyweight && (weightValue === undefined || weightValue === 0) && weightValueString === '') {
            weightValue = 0;
        }
  ...
  const hasPotentiallyIncompleteDataForSubmit = (): boolean => {
    ...
    for (const [exerciseId, sets] of logEntries.entries()) {
      const exerciseDetails = exercisesToLog.find(ex => ex.id === exerciseId);
      for (const setDetail of sets) {
        const repsHasValue = String(setDetail.reps).trim() !== '';
        const weightHasValue = setDetail.weight !== undefined && String(setDetail.weight).trim() !== '';
        if (!setDetail.isCompleted && (repsHasValue || weightHasValue)) return true;
        if (setDetail.isCompleted) {
            if (!repsHasValue) return true;
            if (repsHasValue) {
                const repsNum = Number(String(setDetail.reps).trim());
                if (isNaN(repsNum) || repsNum <= 0 || !Number.isInteger(repsNum)) return true;
            }
            if (weightHasValue) {
                const weightNum = Number(String(setDetail.weight).trim());
                 if (isNaN(weightNum) || weightNum < 0 || (weightNum * 10) % 5 !== 0) return true;
            } else if (!exerciseDetails?.isBodyweight && !weightHasValue) return true;
        }
      }
    }
    return false;
  ...
  const hasEnteredData = useCallback((): boolean => {
    for (const sets of logEntries.values()) {
      for (const setDetail of sets) {
        if (String(setDetail.reps).trim() !== '' || (setDetail.weight !== undefined && String(setDetail.weight).trim() !== '')) return true;
      }
    }
    if (!isNewSession && logForReferenceOrEdit) {
  ...
  const formatPreviousSetDisplay = (setData: SetDetail, exerciseIsBodyweight?: boolean): string => {
    if (exerciseIsBodyweight && (setData.weight === 0 || setData.weight === undefined || String(setData.weight).trim() === '0')) {
        return `${setData.reps} reps (KV)`;
    }
    return `${setData.reps} reps @ ${setData.weight || 0} kg`;
  };
  ...
  return (
    <div className="relative">
      ...
      <div className={`space-y-4 ${isLoadingAiSuggestion ? 'opacity-20' : 'transition-opacity duration-300'}`}>
                                  ...
                                  {Array.from({ length: maxSetsInSuperset }).map((_, roundIndex) => (
                                      <div key={`superset-${exercise.supersetIdentifier}-round-${roundIndex}`} className="mt-2 pt-2 border-t border-gray-200 first:border-t-0 first:mt-0">
                                          ...
                                          {exercisesInThisSuperset.map(exInSS => {
                                              ...
                                              const repsInputId = `reps-${exInSS.id}-${setDetail.id}`;
                                              const weightInputId = `weight-${exInSS.id}-${setDetail.id}`;
                                              ...
                                              return (
                                                  <div key={exInSS.id + '-' + setDetail.id} className={`p-2 mt-2 rounded-lg border-2 transition-colors duration-150 ease-in-out ${setDetail.isCompleted ? 'bg-green-50 border-green-300' : 'bg-white border-gray-300'}`}>
                                                       <p className="text-base font-semibold text-gray-700">{exInSS.name} {exInSS.isBodyweight && <span className="text-sm text-green-600">(KV)</span>}</p>
                                                      <div className="grid grid-cols-[1fr_1fr_auto] gap-x-1.5 items-end mt-0.5">
                                                          <Input inputSize="sm" id={repsInputId} name={repsInputId} type="number" inputMode="numeric" pattern="[0-9]*" placeholder="Reps" value={setDetail.reps} onChange={(e) => handleSetInputChange(exInSS.id, setDetail.id, 'reps', e.target.value)} min="0" step="1" />
                                                          <Input inputSize="sm" id={weightInputId} name={weightInputId} type="number" inputMode="decimal" placeholder="Vikt" value={setDetail.weight} onChange={(e) => handleSetInputChange(exInSS.id, setDetail.id, 'weight', e.target.value)} min="0" step="0.5" />
                                                          <button onClick={() => handleSetCompletionChange(exInSS.id, setDetail.id, !setDetail.isCompleted)} className={`p-1 rounded-md transition-colors ${setDetail.isCompleted ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-gray-300 hover:bg-gray-400 text-gray-700'}`} aria-pressed={setDetail.isCompleted} title={setDetail.isCompleted ? "Markera som ej slutfört" : "Markera som slutfört"}>
                                                              {setDetail.isCompleted ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                                                          </button>
                                                      </div>
                                                      {previousSetDataForDisplaySS && ( <p className="text-sm text-gray-500 mt-1">Föregående: {formatPreviousSetDisplay(previousSetDataForDisplaySS, exInSS.isBodyweight)}</p> )}
                                                  </div>
                                              );
                                          })}
                                      </div>
                                    ...
                                    return (
                                      <div key={setDetail.id} className={`p-2 rounded-lg border-2 transition-colors duration-150 ease-in-out ${setDetail.isCompleted ? 'bg-green-50 border-green-300' : 'bg-white border-gray-300'}`}>
                                        <div className="flex items-center mb-1.5">
                                            <div className="w-24 flex-shrink-0"> {/* Width approx matches button size */}
                                            ...
                                            <div className="w-24 flex-shrink-0" aria-hidden="true"></div> {/* Spacer to balance the button and center the text */}
                                        </div>
                                        <div className="grid grid-cols-[1fr_1fr_auto] gap-x-1.5 items-end">
                                          <Input inputSize="sm" id={repsInputId} name={repsInputId} type="number" inputMode="numeric" pattern="[0-9]*" placeholder="Reps" value={setDetail.reps} onChange={(e) => handleSetInputChange(exercise.id, setDetail.id, 'reps', e.target.value)} min="0" step="1" />
                                          <Input inputSize="sm" id={weightInputId} name={weightInputId} type="number" inputMode="decimal" placeholder="Vikt" value={setDetail.weight} onChange={(e) => handleSetInputChange(exercise.id, setDetail.id, 'weight', e.target.value)} min="0" step="0.5" />
                                          <button onClick={() => handleSetCompletionChange(exercise.id, setDetail.id, !setDetail.isCompleted)} className={`p-1 rounded-md transition-colors ${setDetail.isCompleted ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-gray-300 hover:bg-gray-400 text-gray-700'}`} aria-pressed={setDetail.isCompleted} title={setDetail.isCompleted ? "Markera som ej slutfört" : "Markera som slutfört"}>
                                            {setDetail.isCompleted ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                                          </button>
                                        </div>
                                        {previousSetDataForDisplay && (<p className="text-sm text-gray-500 mt-1">Föregående: {formatPreviousSetDisplay(previousSetDataForDisplay, exercise.isBodyweight)}</p>)}
                                      </div>
                                    )
                                  })}
        ...
        {showSubmitConfirmationModal && ( <ConfirmationModal isOpen={showSubmitConfirmationModal} onClose={handleCancelSubmitConfirmation} onConfirm={handleConfirmSubmit} title="Bekräfta Spara" message={ <div><p className="mb-2">Vissa set är ifyllda med reps/vikt men inte markerade som slutförda, ELLER så är vissa slutförda set inte korrekt ifyllda (t.ex. saknar reps, eller reps är inte heltal, eller vikt är inte hel/halv kilo).</p><p className="font-semibold">Endast korrekt ifyllda och "slutförda" set sparas.</p><p>Vill du fortsätta och spara?</p></div>} confirmButtonText="Ja, spara ändå" confirmButtonVariant="primary" /> )}
      </div>
    </div>
  );
};
