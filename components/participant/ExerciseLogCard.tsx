...
const CheckmarkButton: React.FC<{ isCompleted: boolean; onClick: () => void; }> = ({ isCompleted, onClick }) => {
  const baseClasses = 'w-12 h-12 flex items-center justify-center rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-200';
...
const DeleteSetButton: React.FC<{ onClick: () => void; }> = ({ onClick }) => (
    <button type="button" onClick={onClick} className="w-12 h-12 flex items-center justify-center rounded-lg bg-slate-200 text-slate-600 hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 transition-colors duration-200">
...
interface ExerciseLogCardProps {
  ...
  logForReferenceOrEdit?: WorkoutLog;
  aiExerciseTip?: string;
}

export const ExerciseLogCard: React.FC<ExerciseLogCardProps> = ({ exercise, logEntries, handleUpdateSet, setSetToRemove, logForReferenceOrEdit, aiExerciseTip }) => {
  const sets = logEntries.get(exercise.id) || [];
  const previousSetsForExercise = logForReferenceOrEdit?.entries.find(e => e.exerciseId === exercise.id)?.loggedSets;
                      ...
                      <div className="flex items-center gap-3">
                          <div className="flex-grow flex flex-wrap gap-3">
                            {metricsToLog.map(metric => {
                                ...
                                return (
                                    <div key={metric} className="flex-1 min-w-[120px]">
                                        <Input
                                            type="text"
                                            ...
                                            className="h-16 text-2xl text-center w-full"
                                            inputSize="lg"
                                        />
                                    </div>
                                );
