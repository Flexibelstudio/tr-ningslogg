...
export const PostWorkoutSummaryModal: React.FC<PostWorkoutSummaryModalProps> = ({
  ...
  const totalDuration = summaryData.totalDurationSeconds || 0;
  const minutes = Math.floor(totalDuration / 60);
  const seconds = totalDuration % 60;
  const durationString = `${minutes > 0 ? `${minutes} min ` : ''}${seconds > 0 ? `${seconds} sek` : ''}`.trim();
    ...
    <Modal isOpen={isOpen} onClose={handleModalCloseWithFinalize} title="" size="lg" showCloseButtonOnly={true}>
      <div ref={shareableContentRef} className="bg-white p-4 rounded-lg">
        <div className="text-center space-y-2 mb-6 border-b pb-4">
            <p className="text-lg text-gray-500 font-semibold">Pass Slutfört!</p>
            <h2 className="text-3xl font-bold text-gray-800">{workout.title}</h2>
        </div>
        <div className="text-center space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
              {summaryData.totalDistanceMeters && summaryData.totalDistanceMeters > 0 && (
                  <div>
                      <p className="text-base text-gray-500 uppercase tracking-wide">Total Distans</p>
                      <p className="text-4xl font-bold text-flexibel">
                          {summaryData.totalDistanceMeters.toLocaleString('sv-SE')} <span className="text-2xl font-medium text-gray-500">m</span>
                      </p>
                  </div>
              )}
              {summaryData.totalDurationSeconds && summaryData.totalDurationSeconds > 0 && (
                  <div>
                      <p className="text-base text-gray-500 uppercase tracking-wide">Total Tid</p>
                      <p className="text-4xl font-bold text-flexibel">
                          {durationString}
                      </p>
                  </div>
              )}
              {summaryData.totalCaloriesKcal && summaryData.totalCaloriesKcal > 0 && (
                  <div>
                      <p className="text-base text-gray-500 uppercase tracking-wide">Totala Kalorier</p>
                      <p className="text-4xl font-bold text-flexibel">
                          {Math.round(summaryData.totalCaloriesKcal).toLocaleString('sv-SE')} <span className="text-2xl font-medium text-gray-500">kcal</span>
                      </p>
                  </div>
              )}
            </div>
            ...
            {summaryData.animalEquivalent && (
              <div className="p-4 bg-teal-50 rounded-lg border border-teal-200 flex flex-col items-center">
                <span className="text-6xl sm:text-7xl block mb-2" aria-hidden="true">{summaryData.animalEquivalent.emoji || '💪'}</span>
          ...
          {summaryData.weightOnlyAchievements && summaryData.weightOnlyAchievements.length > 0 && (
            <div className="mt-6 pt-4 border-t">
              <h3 className="text-xl font-semibold text-gray-800 mb-3">⚖️ Loggade Vikter</h3>
              <ul className="space-y-2 text-left">
                {summaryData.weightOnlyAchievements.map((item, index) => (
                  <li key={index} className="p-3 bg-gray-100 border border-gray-300 rounded-md text-base flex justify-between">
                    <span className="font-semibold text-gray-700">{item.exerciseName}</span>
          ...
          {summaryData.newBaselines && summaryData.newBaselines.length > 0 && (
            <div className="mt-6 pt-4 border-t">
              <h3 className="text-xl font-semibold text-gray-800 mb-3">📊 Ny Baslinje Satt!</h3>
              <ul className="space-y-2 text-left">
          ...
          {summaryData.newPBs && summaryData.newPBs.length > 0 && (
            <div className="mt-6 pt-4 border-t">
              <h3 className="text-xl font-semibold text-gray-800 mb-3">🏆 Nya Rekord!</h3>
              <ul className="space-y-2 text-left">
