...
const ScaleSelector: React.FC<ScaleSelectorProps> = ({ label, options, selectedValue, onChange, ariaLabel }) => {
  return (
    <div>
      <label className="block text-sm sm:text-base font-medium text-gray-700 mb-2">{label}</label>
      <div className="flex flex-wrap justify-around items-center p-2 bg-gray-50 rounded-lg shadow-sm" role="radiogroup" aria-label={ariaLabel || label}>
        {options.map((option) => (
          <button
            key={option.value}
            type="button"
            onClick={() => onChange(option.value)}
            className={`p-1.5 sm:p-2 m-1 rounded-full transition-all duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-flexibel
            ...
            title={option.label}
          >
            <span className="text-2xl sm:text-3xl" aria-hidden="true">{option.emoji}</span>
          </button>
        ))}
      </div>
    </div>
...
export const MentalWellbeingForm = forwardRef<MentalWellbeingFormRef, MentalWellbeingFormProps>(({
  ...
  const handleSave = useCallback(() => {
    ...
    const newWellbeingData: ParticipantMentalWellbeing = {
      id: currentWellbeing?.id || participantId!,
      stressLevel,
      energyLevel,
      sleepQuality,
      overallMood,
