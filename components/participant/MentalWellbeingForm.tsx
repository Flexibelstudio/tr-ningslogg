      ...
      <div className="flex flex-wrap justify-around items-center p-2 bg-gray-50 rounded-lg shadow-sm" role="radiogroup" aria-label={ariaLabel || label}>
        {options.map((option) => (
          <button
            ...
            className={`p-2 sm:p-3 m-1 rounded-full transition-all duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-flexibel
            ...
            title={option.label}
          >
            <span className="text-3xl sm:text-4xl" aria-hidden="true">{option.emoji}</span>
          </button>
        ))}
      </div>
    </div>
