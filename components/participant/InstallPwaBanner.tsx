...
export const InstallPwaBanner: React.FC<InstallPwaBannerProps> = ({ participantProfile }) => {
  ...
  useEffect(() => {
    const isProfileComplete = !!(participantProfile?.name && participantProfile?.age && participantProfile?.gender && participantProfile.gender !== '-');
    const isDismissed = Date.now() < dismissedUntil;
    
    // The banner should be visible if...
    if (canInstall && !isStandalone && isProfileComplete && !isDismissed) {
      setIsVisible(true);
    } else {
      setIsVisible(false);
    }
  }, [canInstall, isStandalone, participantProfile, dismissedUntil]);
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-white/95 backdrop-blur-sm border-t border-gray-200 animate-fade-in-down">
        <div className="container mx-auto max-w-4xl flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
             <img src="/icon-192x192.png" alt="App icon" className="w-12 h-12 hidden sm:block"/>
             <div>
