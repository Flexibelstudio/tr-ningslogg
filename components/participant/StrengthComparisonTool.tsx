...
import { ParticipantProfile, UserStrengthStats, StrengthStandard, LiftType, StrengthLevel, StrengthStandardDetail, AllUserProvidedStrengthMultipliers, GenderOption, ParticipantGoalData } from '../../types';
...
import { getPassRecommendations } from '../../utils/passRecommendationHelper'; // Import the new helper
...
const LIFT_CONFIG: { lift: LiftType, label: string, icon: () => JSX.Element, statKey: keyof UserStrengthStats }[] = [
...
interface StrengthComparisonToolProps {
  ...
  strengthStats: UserStrengthStats | null;
...
export const StrengthComparisonTool = forwardRef<StrengthComparisonToolRef, StrengthComparisonToolProps>(({
  ...
  const [bodyweight, setBodyweight] = useState<string>(strengthStats?.bodyweightKg?.toString() || '');
  const [oneRMs, setOneRMs] = useState<{[key in LiftType]?: string}>(() => {
    const initialRMs: {[key in LiftType]?: string} = {};
    LIFT_CONFIG.forEach(config => {
        const val = strengthStats?.[config.statKey] as number | undefined;
  ...
  useEffect(() => {
    setBodyweight(strengthStats?.bodyweightKg?.toString() || '');
  ...
  const handleSave = useCallback(() => {
    ...
    const newStats: UserStrengthStats = {
      id: strengthStats?.id || profile.id, 
    ...
    onSaveStrengthStats(newStats);
    return true;
  }, [bodyweight, oneRMs, profile, strengthStats, onSaveStrengthStats, validateNumericInput]);
