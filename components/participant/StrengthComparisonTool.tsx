...
export const StrengthComparisonTool = forwardRef<StrengthComparisonToolRef, StrengthComparisonToolProps>(({
  ...
  const areAllStatsFilled = useMemo(() => {
    return !!(
        ...
        squat1RMax.trim() && 
        benchPress1RMax.trim() && 
        deadlift1RMax.trim() && 
        overheadPress1RMax.trim()
    );
  ...
  MAIN_LIFTS_CONFIG_HEADER.forEach(lift => {
      const stateValue = { squat1RMaxKg: squat1RMax, benchPress1RMaxKg: benchPress1RMax, deadlift1RMaxKg: deadlift1RMax, overheadPress1RMaxKg: overheadPress1RMax }[lift.statKey];
      if (!stateValue.trim()) {
    ...
    <div className="space-y-6">
      {areAllStatsFilled ? (
        <div className="space-y-4">
            ...
            <Button onClick={handleShare} fullWidth variant="secondary">Dela resultat</Button>
        </div>
      ) : (
        !profile.bodyweightKg ? (
            <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-lg">
                ...
                <Button onClick={onOpenPhysiqueModal} className="mt-3">Ange kroppsvikt</Button>
            </div>
        ) : (
                     ...
                     <ul className="mt-3 space-y-2">
                        {missingStats.map(stat => (
                            <li key={stat.key} className="flex items-center text-base">
                                ...
                                <button onClick={() => inputRefs[stat.key].current?.focus()} className="ml-auto text-sm text-flexibel hover:underline">Fyll i</button>
                            </li>
                        ))}
                    </ul>
                )}
        ...
        <h3 className="text-xl font-semibold text-gray-700">Dina 1RM</h3>
         {profile?.bodyweightKg && (
            <div className="p-3 bg-gray-100 rounded-lg flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={onOpenPhysiqueModal}>Ändra</Button>
            </div>
        )}

        {MAIN_LIFTS_CONFIG_HEADER.map(({ lift, statKey, label }) => {
            const liftState = { squat1RMaxKg: squat1RMax, benchPress1RMaxKg: benchPress1RMax, deadlift1RMaxKg: deadlift1RMax, overheadPress1RMaxKg: overheadPress1RMax }[statKey];
            const setStateAction = { squat1RMaxKg: setSquat1RMax, benchPress1RMaxKg: setBenchPress1RMax, deadlift1RMaxKg: setDeadlift1RMax, overheadPress1RMaxKg: setOverheadPress1RMax }[statKey];
            ...
            return (
              <details key={statKey} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200" open={expandedLifts[lift]}>
                  <summary className="font-semibold text-lg text-gray-800 cursor-pointer list-none flex justify-between items-center" onClick={(e) => { e.preventDefault(); setExpandedLifts(p => ({...p, [lift]: !p[lift]})); }}>
                      <span>{lift}: <span className="font-bold text-flexibel">{liftState || '-'} kg</span></span>
                      <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-gray-500 transition-transform ${expandedLifts[lift] ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                  </summary>
                  <div className="mt-3 pt-3 border-t space-y-4">
                      {liftScoreData && (
                           <div className="p-3 border rounded-md bg-gray-50 text-center">
                                ...
                                <p className="text-3xl font-bold" style={{ color: LEVEL_COLORS_HEADER[liftScoreData.level] }}>{liftScoreData.score}</p>
                                <p className="text-lg font-semibold" style={{ color: LEVEL_COLORS_HEADER[liftScoreData.level] }}>{liftScoreData.level}</p>
                            </div>
                      )}
                      <FocusedClubDisplay liftType={lift} oneRepMax={Number((liftState || '0').replace(',', '.')) || 0} />
                      <Input label={`Uppdatera ${label}`} id={statKey} name={statKey} type="number" step="0.5" value={liftState} onChange={(e) => { setStateAction(e.target.value); validateField(e.target.value, statKey); }} error={errors[statKey]} ref={inputRefs[statKey]} />
                  </div>
              </details>
            );
          })}
