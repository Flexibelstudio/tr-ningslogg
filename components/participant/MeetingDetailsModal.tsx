...
export const MeetingDetailsModal: React.FC<MeetingDetailsModalProps> = ({ 
    isOpen, onClose, session, coach, currentUserId, onAddComment, onDeleteComment, readOnlyComments = false 
}) => {
    if (!isOpen || !session) return null;

    const handleAddComment = (text: string) => {
        onAddComment(session.id, 'one_on_one_session', text);
    };

    const handleDeleteComment = (commentId: string) => {
        onDeleteComment(session.id, 'one_on_one_session', commentId);
    };
