...
export const CommunityModal: React.FC<CommunityModalProps> = ({
  ...
  const currentParticipant = useMemo(() => allParticipants.find(p => p.id === currentParticipantId), [allParticipants, currentParticipantId]);
  ...
  const incomingRequests = useMemo(() => {
    return connections
        ...
        .map(conn => {
            ...
            return requester ? { ...conn, requesterName: requester.name || 'Okänd' } : null;
        })
        .filter(Boolean) as (Connection & { requesterName: string })[];
  }, [connections, allParticipants, currentParticipantId]);
  
  const searchResults = useMemo(() => {
    if (!searchTerm.trim() || !currentParticipant?.locationId) return [];
    const lowerCaseSearch = searchTerm.toLowerCase();
    const currentUserLocationId = currentParticipant.locationId;

    return allParticipants.filter(p => 
        p.id !== currentParticipantId &&
        p.isSearchable && 
        p.locationId === currentUserLocationId && // Filter by location
        p.name?.toLowerCase().includes(lowerCaseSearch)
    );
  }, [searchTerm, allParticipants, currentParticipantId, currentParticipant]);
                ...
                <ul className="mt-4 space-y-2 max-h-80 overflow-y-auto">
                     {incomingRequests.length > 0 ? incomingRequests.map(r => (
                        <li key={r.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-md">
                            <span className="font-medium text-lg">{r.requesterName}</span>
