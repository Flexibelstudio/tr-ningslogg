...
import { ActivityLog, Workout, WorkoutLog, GeneralActivityLog, Exercise, GoalCompletionLog, ParticipantGoalData, UserStrengthStat, ParticipantConditioningStat, ParticipantClubMembership, ParticipantProfile, CoachEvent } from '../../types';
...
export const DayActivitiesModal: React.FC<DayActivitiesModalProps> = ({
  ...
  const specialEventsForDay = useMemo(() => {
    ...
    const coachEventsToday = coachEvents.filter(e => dateUtils.isSameDay(new Date(e.date), selectedDate));
    ...
    if (participantProfile?.lastUpdated && dateUtils.isSameDay(new Date(participantProfile.lastUpdated), selectedDate) && (participantProfile.inbodyScore || participantProfile.muscleMassKg)) {
    ...
    return events;
  }, [selectedDate, allParticipantGoals, clubMemberships, participantProfile, strengthStatsHistory, conditioningStatsHistory, coachEvents]);
        ...
        <div className="space-y-4">
          {specialEventsForDay.length > 0 && (
              <div className="p-3 bg-gray-100 rounded-lg space-y-2 border">
                  <h4 className="text-sm font-semibold text-gray-600 uppercase">Händelser denna dag</h4>
                      ...
                      {specialEventsForDay.map((event, index) => (
                          <li key={index} className="flex items-start text-base">
                              ...
                              <div className="flex-1">
                                  ...
                                  {event.details && <p className="text-sm text-gray-500 italic">{event.details}</p>}
                              </div>
                          </li>
                      ))}
                  </ul>
          ...
          {hasGoalTargetForDay && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 flex items-center text-blue-800">
                  ...
                  <span className="font-semibold">Detta är ett måldatum!</span>
              </div>
          )}
          {sortedActivities.length === 0 && specialEventsForDay.length === 0 && !hasGoalTargetForDay ? (
                ...
                if (log.type === 'workout') {
                  ...
                  return (
                    ...
                    return (
                      ...
                      >
                        <div className="flex-grow">
                          <p className={`text-lg font-semibold ${isSelected ? 'text-yellow-800' : 'text-yellow-700'}`}>
                              🏆 Mål Uppnått!
                          </p>
