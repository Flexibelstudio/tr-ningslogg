...
import { ActivityLog, Workout, WorkoutLog, GeneralActivityLog } from '../../types';
...
interface DayActivitiesModalProps {
  ...
  workouts: Workout[]; // To get workout titles
...
export const DayActivitiesModal: React.FC<DayActivitiesModalProps> = ({
  ...
  const getConfirmationMessage = () => {
    ...
    if (activityToConfirmDelete.type === 'workout') {
        const workoutTemplate = workouts.find(w => w.id === (activityToConfirmDelete as WorkoutLog).workoutId);
                ...
                if (log.type === 'workout') {
                  ...
                  return (
                    <li 
                      ...
                      aria-label={`Välj logg för ${workoutTemplate?.title || "Okänt Gympass"}`}
                    >
                      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                        <div className="flex-grow">
                          <p className={`text-lg font-semibold ${isSelected ? 'text-flexibel' : 'text-gray-800'}`}>
                            🏋️ {workoutTemplate?.title || "Okänt Gympass"} {moodEmoji && <span className="ml-1 text-2xl">{moodEmoji}</span>}
                          </p>
                        ...
                        <Button
                            onClick={(e) => { e.stopPropagation(); onViewLogSummary(log as WorkoutLog);}}
                            variant="outline"
                            size="sm"
                            className="w-full sm:w-auto mt-2 sm:mt-0 flex-shrink-0"
                            aria-label={`Visa detaljer för ${workoutTemplate?.title || "Okänt Gympass"}`}
                        >
                            Visa/Redigera
                        </Button>
                      </div>
