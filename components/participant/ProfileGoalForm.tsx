...
export const ProfileGoalForm: React.FC<ProfileGoalFormProps> = ({
  ...
  const handleSaveClick = () => {
    ...
    if (ageError || muscleMassError || fatMassError || inbodyScoreError || targetDateError) {
    ...
    setHasSaved(true);
    setTimeout(() => {
      onSave(profileData, goalDataToSave, markGoalCompleted, calculatedNoGoalAdviseOptOut);
  ...
  const isFormValidToSave = () => {
    if (ageError || muscleMassError || fatMassError || inbodyScoreError || targetDateError) return false;
    ...
    <div className="space-y-6 py-4">
      ...
      <section className="space-y-4 pt-4 border-t">
        ...
        <div className="mt-4 bg-violet-50 p-3 rounded-lg border border-violet-200">
            <Button
                onClick={onTriggerAiGoalPrognosis}
                variant="outline"
                size="sm"
                fullWidth
