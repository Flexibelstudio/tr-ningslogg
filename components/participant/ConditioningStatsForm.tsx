...
export const ConditioningStatsForm = forwardRef<ConditioningStatsFormRef, ConditioningStatsFormProps>(({
  ...
  const findPreviousValue = useCallback((key: keyof Omit<ParticipantConditioningStat, 'id' | 'lastUpdated'| 'participantId'>) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <div className="space-y-2 md:col-span-2">
            ...
            {findPreviousValue('rower2000mTimeSeconds') && (
                <p className="text-xs text-gray-500 px-1">
                    Föregående: {Math.floor((findPreviousValue('rower2000mTimeSeconds')?.value as number) / 60)} min { (findPreviousValue('rower2000mTimeSeconds')?.value as number) % 60 } sek (
