...
import {
    ...
    UserRole, ParticipantMentalWellbeing, WorkoutBlock, WorkoutCategory, Exercise, SetDetail, ParticipantGamificationStats, GoalCompletionLog
} from '../../types';
...
interface ParticipantAreaProps {
  ...
  setParticipantGoals: (goals: ParticipantGoalData[] | ((prevGoals: ParticipantGoalData[]) => ParticipantGoalData[])) => void;
  generalActivityLogs: GeneralActivityLog[];
  setGeneralActivityLogs: (logs: GeneralActivityLog[] | ((prevLogs: GeneralActivityLog[]) => GeneralActivityLog[])) => void;
  goalCompletionLogs: GoalCompletionLog[];
  setGoalCompletionLogs: (logs: GoalCompletionLog[] | ((prevLogs: GoalCompletionLog[]) => GoalCompletionLog[])) => void;
...
export const ParticipantArea: React.FC<ParticipantAreaProps> = ({
  ...
  const allActivityLogs = useMemo<ActivityLog[]>(() => {
    return [...workoutLogs, ...generalActivityLogs, ...goalCompletionLogs].sort((a, b) => new Date(b.completedDate).getTime() - new Date(a.completedDate).getTime());
  ...
  const handleSaveLog = (logData: WorkoutLog) => {
    ...
    const logWithParticipantId: WorkoutLog = {
        ...logData,
        participantId: participantProfile.id,
    };

    const existingLogIndex = workoutLogs.findIndex(l => l.id === logWithParticipantId.id);
    let updatedWorkoutLogsList;
    if (existingLogIndex > -1) {
      updatedWorkoutLogsList = workoutLogs.map((l, index) => index === existingLogIndex ? logWithParticipantId : l);
    } else {
      updatedWorkoutLogsList = [...workoutLogs, logWithParticipantId];
    }

    let finalLogDataWithSummary = { ...logWithParticipantId };
    
    if (logWithParticipantId.entries.length > 0) { 
        const summary = calculatePostWorkoutSummary(logWithParticipantId, workouts);
        finalLogDataWithSummary = { ...logWithParticipantId, postWorkoutSummary: summary };
    }
    ...
    const handleSaveProfileAndGoals = (
        ...
        if (migratedWorkoutCount !== undefined) {
            setParticipantGamificationStats(prev => {
               const baseStats = prev || { id: profileData.id, longestStreakWeeks: 0, lastUpdated: new Date().toISOString() };
               return {
                   ...baseStats,
                   id: profileData.id,
                   migratedWorkoutCount: migratedWorkoutCount,
...
Om du behöver hänvisa till en mänsklig coach, använd formuleringar som 'ta upp detta med din coach i studion'.`;
  ...
  const handleTriggerAiGoalPrognosis = useCallback(async (goalDataOverride?: Omit<ParticipantGoalData, 'id' | 'participantId' | 'currentWeeklyStreak' | 'lastStreakUpdateEpochWeekId' | 'setDate'| 'isCompleted' | 'completedDate'>) => {
    ...
    const prompt = `
      System: Du är "Flexibot", en AI-coach som ger realistiska och peppande prognoser baserat på ett nyligen satt mål. Du ger ALDRIG ett exakt datum, utan använder alltid en tidsram (t.ex. "inom 3-5 månader"). Svara alltid på svenska.

      DINA REGLER FÖR INNEHÅLL:
      - **OM det finns LITE eller INGEN träningsdata ('Sammanfattning av de 5 senaste loggade aktiviteterna' är tom eller har färre än 3 loggar), MÅSTE du:**
          - **Inleda ditt svar med:** "Kul med ett nytt mål! Detta är en första prognos baserad på det du angett. Ju mer du loggar och fyller i din profil (under 'Styrka' och 'Kondition'), desto mer pricksäker blir min feedback."
          - **Göra din prognos mer generell och villkorad.** Exempel: "Om du är konsekvent med dina pass kan du se resultat inom X månader."
          - **Avsluta med en uppmuntran** till medlemmen att fylla i mer data för att få bättre feedback, t.ex. "Börja med att logga ditt nästa pass och fyll i dina styrkestatistiker för att vi ska kunna följa din resa på bästa sätt!".
      ...
      1.  **## Prognos för ditt mål: "${goalToAnalyze.fitnessGoals.substring(0, 40)}..."**
      ...
      3.  **Jämförelse med ditt Måldatum:**
          ...
          *   Exempel på tonfall:
              *   Om medlemmens datum är **realistiskt**: "Ditt satta måldatum den ${goalToAnalyze.targetDate ? new Date(goalToAnalyze.targetDate).toLocaleDateString('sv-SE') : ''} ser ut att vara **mycket realistiskt** med tanke på vår prognos. Fortsätt med din nuvarande plan!"
              *   Om medlemmens datum är **ambitiöst**: "Ditt satta måldatum den ${goalToAnalyze.targetDate ? new Date(goalToAnalyze.targetDate).toLocaleDateString('sv-SE') : ''} är **ambitiöst men möjligt** om du är väldigt dedikerad. Det kommer kräva extra fokus på [faktor, t.ex. kost eller konsistens]."
              *   Om medlemmens datum är **väldigt orealistiskt**: "Ditt satta måldatum den ${goalToAnalyze.targetDate ? new Date(goalToAnalyze.targetDate).toLocaleDateString('sv-SE') : ''} är **väldigt utmanande**. Baserat på din nuvarande data kan det vara svårt att nå dit så snabbt. En mer hållbar tidsram kan vara [AI:s prognos]. Prata gärna med din coach i studion om hur ni kan optimera din plan."
          *   **Om medlemmen INTE har satt ett datum, hoppa över denna punkt helt.**
      4.  **Viktiga Faktorer:** Förklara KORT vilka faktorer som påverkar tidsramen. Exempel: "* **Konsistens:** Att du håller i dina ${goalToAnalyze.workoutsPerWeekTarget} pass/vecka är avgörande. * **Intensitet & Progression:** Att du successivt ökar vikter/reps. * **Kost & Återhämtning:** Spelar en stor roll utanför gymmet."
  ...
  const handleLogGeneralActivity = (activityData: Omit<GeneralActivityLog, 'id' | 'completedDate' | 'type' | 'participantId'>) => {
    if (!participantProfile?.id) {
        alert("Profilinformation saknas. Kan inte spara logg.");
        return;
    }
  ...
  const overallStrengthLevel = useMemo(() => {
    ...
    const totalWorkouts = useMemo(() => (participantGamificationStats?.migratedWorkoutCount || 0) + allActivityLogs.length, [allActivityLogs, participantGamificationStats]);
    ...
    <div className="flex flex-col h-full" ref={mainContentRef}>
                        ...
                        content: (
                            <div className="space-y-2">
                                ...
                                <ul className="list-disc pl-5 mt-2 space-y-1">
                                    <li><strong>Totalt antal pass:</strong> Alla dina loggade gympass och andra aktiviteter, inklusive de du eventuellt fört över från en annan app.</li>
      ...
      {isProfileGoalModalOpen && (
        <ProfileGoalModal
          ...
          allParticipantGoals={participantGoals}
          participantGamificationStats={participantGamificationStats}
          onSave={(profileDataFromForm, goalDataFromForm, markLatestGoalAsCompleted, noGoalAdviseOptOutIgnored, migratedWorkoutCount) => {
            ...
            handleSaveProfileAndGoals(newProfile, goalDataFromForm, markLatestGoalAsCompleted, migratedWorkoutCount);
