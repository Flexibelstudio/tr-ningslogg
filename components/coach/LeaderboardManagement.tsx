...
export const LeaderboardManagement: React.FC<LeaderboardManagementProps> = ({ 
    ...
    const highestLiftClubByParticipant = useMemo(() => {
        const highestClubs = new Map<string, Map<LiftType, string>>();
        const liftClubs = CLUB_DEFINITIONS.filter(c => c.type === 'LIFT' || c.type === 'BODYWEIGHT_LIFT');

        for (const participant of optedInParticipants) {
            const participantClubs = new Map<LiftType, { id: string, value: number }>();
            const membershipsForParticipant = clubMemberships.filter(m => m.participantId === participant.id);
            
            for (const membership of membershipsForParticipant) {
                const clubDef = liftClubs.find(c => c.id === membership.clubId);
                if (clubDef && clubDef.liftType) {
                    const clubValue = clubDef.threshold || (clubDef.multiplier || 0) * (participant.bodyweightKg || 70);
                    const currentHighest = participantClubs.get(clubDef.liftType);
                    if (!currentHighest || clubValue > currentHighest.value) {
                        participantClubs.set(clubDef.liftType, { id: clubDef.id, value: clubValue });
                    }
                }
            }
            
            const finalMap = new Map<LiftType, string>();
            participantClubs.forEach((value, key) => finalMap.set(key, value.id));
            highestClubs.set(participant.id, finalMap);
        }
        return highestClubs;
    }, [optedInParticipants, clubMemberships]);
                            ...
                            {CLUB_DEFINITIONS.map(club => {
                                const members = optedInParticipants.filter(p => {
                                    if (club.type === 'LIFT' || club.type === 'BODYWEIGHT_LIFT') {
                                        if (!club.liftType) return false;
                                        return highestLiftClubByParticipant.get(p.id)?.get(club.liftType) === club.id;
                                    }
                                    // For non-lift clubs, keep original logic of just checking for membership
                                    return clubMemberships.some(m => m.clubId === club.id && m.participantId === p.id);
                                });
