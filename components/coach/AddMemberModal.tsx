...
export const AddMemberModal: React.FC<AddEditMemberModalProps> = ({ isOpen, onClose, onSaveMember, memberToEdit, existingEmails, locations, memberships, loggedInStaff }) => {
  ...
  const [errors, setErrors] = useState<{ name?: string, email?: string, locationId?: string, membershipId?: string }>({});
  ...
  const validate = () => {
    const newErrors: { name?: string, email?: string, locationId?: string } = {};
    if (!name.trim()) {
