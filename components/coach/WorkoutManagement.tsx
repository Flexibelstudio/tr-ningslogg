...
export const WorkoutManagement: React.FC<WorkoutManagementProps> = ({ ai, participants }) => {
    const { workouts, setWorkoutsData } = useAppContext();
    ...
    const handleSaveWorkout = (newWorkout: Workout) => {
        setWorkoutsData((prevWorkouts) => sortWorkoutsByCategoryThenTitle([...prevWorkouts, newWorkout]));
    ...
    if (workoutToConfirmDelete) {
        setWorkoutsData(prev => prev.filter(w => w.id !== workoutToConfirmDelete.id));
    }
    ...
    const handleAcceptAiSuggestion = (suggestion: { title: string; coachNote?: string; blocksData: WorkoutBlock[] }) => {
        ...
        setWorkoutsData((prevWorkouts) => sortWorkoutsByCategoryThenTitle([...prevWorkouts, newWorkoutDraft]));
        setIsAiAssistantModalOpen(false); 
      };

      const sortedWorkouts = sortWorkoutsByCategoryThenTitle(templateWorkouts);
