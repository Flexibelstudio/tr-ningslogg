...
export const CreateWorkoutModal: React.FC<CreateWorkoutModalProps> = ({ 
        ...
        const sanitizedBlocks = blocksFromTemplate.map(block => {
          ...
          return {
            ...
            exercises: (block.exercises || []).map(ex => ({ 
              ...
              loggableMetrics: ex.loggableMetrics?.length ? ex.loggableMetrics : ['reps', 'weight'] as LoggableMetric[],
            }))
          };
        });
  ...
  const handleNewExerciseInputChange = (blockId: string, field: keyof Omit<NewExerciseFormState, 'loggableMetrics'>, value: string | LiftType | boolean) => {
    setNewExerciseForms(prev => {
  ...
  const handleMetricToggle = (blockId: string, metric: LoggableMetric) => {
    setNewExerciseForms(prev => {
        const currentFormState = prev[blockId] || { ...initialNewExerciseFormState };
        const currentMetrics = currentFormState.loggableMetrics || [];
        ...
        return {
            ...prev,
            [blockId]: { ...currentFormState, loggableMetrics: newMetrics }
        };
    });
  };
  ...
  const handleAddExerciseToBlock = (blockId: string) => {
    ...
    const newExercise: Exercise = {
      ...
      loggableMetrics: formState.loggableMetrics.length > 0 ? formState.loggableMetrics : ['reps', 'weight'] as LoggableMetric[],
    };
                                        ...
                                        <div className={`p-2.5 bg-white rounded-md border flex justify-between items-start ${selectedIdsInBlock.includes(ex.id) ? 'ring-2 ring-blue-500' : ''}`}>
                                            <div className="flex items-start flex-grow">
                                                ...
                                                <div className="flex-grow">
                                                    ...
                                                    {(ex.loggableMetrics && ex.loggableMetrics.length > 0) && (
                                                        <p className="text-xs text-gray-400 mt-0.5">Loggas med: {ex.loggableMetrics.join(', ')}</p>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    <div className="space-y-2 p-3 border rounded-md bg-white mt-3">
                        ...
                        <div className="mt-2 space-y-1">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {LOGGABLE_METRICS_OPTIONS.map(metricOpt => (
                                    <label key={metricOpt.id} className="flex items-center space-x-2 p-1">
                                        <input
                                            type="checkbox"
                                            checked={currentNewExerciseForm.loggableMetrics.includes(metricOpt.id)}
                                            onChange={() => handleMetricToggle(block.id, metricOpt.id)}
                                            className="h-4 w-4 text-flexibel border-gray-300 rounded focus:ring-flexibel"
                                        />
                                        <span className="text-sm text-gray-700">{metricOpt.label}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
