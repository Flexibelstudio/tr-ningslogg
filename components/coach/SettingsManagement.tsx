...
interface SettingsManagementProps {
  locations: Location[];
  setLocations: (updater: Location[] | ((prev: Location[]) => Location[])) => void;
  ...
  workouts: Workout[];
  setWorkouts: (updater: Workout[] | ((prev: Workout[]) => Workout[])) => void;
...
const AddEditMembershipModal: React.FC<{
    ...
    const [restricted, setRestricted] = useState<Set<WorkoutCategory>>(new Set());
    const [error, setError] = useState('');

    useEffect(() => {
        setName(membershipToEdit?.name || '');
        setDescription(membershipToEdit?.description || '');
        setRestricted(new Set(membershipToEdit?.restrictedCategories || []));
        setError('');
    ...
    const handleSave = () => {
        ...
        const membershipData: Membership = {
            ...
            restrictedCategories: Array.from(restricted),
        };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={membershipToEdit ? 'Redigera Medlemskap' : 'Nytt Medlemskap'}>
            <div className="space-y-4">
                <Input label="Namn *" value={name} onChange={e => setName(e.target.value)} error={error} />
                <Textarea label="Beskrivning" value={description} onChange={e => setDescription(e.target.value)} rows={2} />
                <div>
                    <h4 className="text-base font-medium text-gray-700 mb-2">Begränsa tillgång till passkategorier</h4>
                    <div className="space-y-2 p-3 bg-gray-50 rounded-md border">
                        {workoutCategories.filter(cat => cat.name !== 'Personligt program').map(cat => (
                            <label key={cat.id} className="flex items-center space-x-2">
                                <input
                                    type="checkbox"
                                    checked={restricted.has(cat.name)}
...
export const SettingsManagement: React.FC<SettingsManagementProps> = ({ 
  locations, setLocations, memberships, setMemberships, workoutCategories, setWorkoutCategories, workouts, setWorkouts, participants, setParticipants, integrationSettings, setIntegrationSettings, loggedInStaff
}) => {
    return (
        <div className="space-y-6">
            <LocationManager locations={locations} setLocations={setLocations} />
            <MembershipManager memberships={memberships} setMemberships={setMemberships} workoutCategories={workoutCategories} />
            <WorkoutCategoryManager workoutCategories={workoutCategories} setWorkoutCategories={setWorkoutCategories} />
              ...
              <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                  ...
                  <div className="space-y-4">
                      <ToggleSwitch
                          ...
                          onChange={(isEnabled) =>
                              setIntegrationSettings(prev => ({...prev, isBookingEnabled: isEnabled}))
                          }
                      ...
                      {integrationSettings.isBookingEnabled && (
                          <div className="pl-6 mt-2 border-l-2 ml-3 border-gray-200 space-y-4">
                              <Select
                                  ...
                                  onChange={(e) => setIntegrationSettings(prev => ({ ...prev, bookingLeadTimeWeeks: Number(e.target.value) }))}
                                  options={[
                              ...
                              <Input
                                  ...
                                  onChange={(e) => setIntegrationSettings(prev => ({ ...prev, cancellationCutoffHours: Number(e.target.value) < 0 ? 0 : Number(e.target.value) }))}
                                  inputSize="sm"
                                  containerClassName="max-w-xs"
                                  min="0"
                                  step="1"
