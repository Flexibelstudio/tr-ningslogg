...
const ElksSettingsManager: React.FC = () => {
    ...
    const [localApiId, setLocalApiId] = useState(integrationSettings.elksApiId || '');
    const [localApiSecret, setLocalApiSecret] = useState(integrationSettings.elksApiSecret || '');
    const [newCallerId, setNewCallerId] = useState('');
    const [isSaving, setIsSaving] = useState(false);
    const [saveStatus, setSaveStatus] = useState<'idle' | 'success'>('idle');

    useEffect(() => {
        setLocalApiId(integrationSettings.elksApiId || '');
        setLocalApiSecret(integrationSettings.elksApiSecret || '');
    }, [integrationSettings.elksApiId, integrationSettings.elksApiSecret]);

    const handleSaveApis = () => {
        setIsSaving(true);
        setIntegrationSettingsData(prev => ({ 
            ...prev, 
            elksApiId: localApiId.trim(), 
            elksApiSecret: localApiSecret.trim() 
        }));
    ...
    const handleAddCallerId = () => {
        ...
        if (!current.includes(newCallerId.trim())) {
            setIntegrationSettingsData(prev => ({ 
                ...prev, 
                verifiedCallerIds: [...current, newCallerId.trim()] 
            }));
        }
        setNewCallerId('');
    };
    ...
    const handleRemoveCallerId = (id: string) => {
        ...
        setIntegrationSettingsData(prev => ({ 
            ...prev, 
            verifiedCallerIds: current.filter(c => c !== id) 
        }));
    };
    ...
    return (
        <Card title="46elks Integration">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 mb-6">
                <p className="text-sm text-blue-800 font-semibold mb-1">Var hittar jag mina nycklar?</p>
                <p className="text-xs text-blue-700 leading-relaxed">
                    1. Logga in på 46elks.se.<br/>
                    2. Klicka på fliken <strong>Account</strong> längst upp till höger.<br/>
                    3. Leta efter sektionen <strong>API Credentials</strong>.<br/>
                    4. Kopiera <strong>API Username</strong> (börjar på 'u') till första fältet nedan.<br/>
                    5. Kopiera <strong>API Password</strong> till fältet för API Secret.
                </p>
            </div>
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Input 
                        ...
                        placeholder="Ditt API Password"
                    />
                </div>
                
                <div className="flex justify-end items-center gap-3 pt-2">
                    {saveStatus === 'success' && <span className="text-green-600 font-medium text-sm animate-fade-in">✓ Uppgifter sparade!</span>}
                    <Button onClick={handleSaveApis} disabled={isSaving}>
                        {isSaving ? 'Sparar...' : 'Spara API-uppgifter'}
                    </Button>
                </div>
                
                <div className="pt-6 border-t">
                    <h4 className="font-semibold text-gray-700 mb-2">Verifierade Caller IDs (studionummer)</h4>
                    <p className="text-xs text-gray-500 mb-2">Lägg till nummer som kunden ska se som avsändare. Numret måste först verifieras i 46elks portal.</p>
                    <div className="flex gap-2 mb-3">
                        ...
                        />
                        <Button onClick={handleAddCallerId} variant="outline">Lägg till nummer</Button>
                    </div>
