...
2.  **## Detailed Program**
    ...
    *   För VARJE övning, direkt efter namnet, använd ett kolon med mellanslag (": ") följt av detaljerna. Dessa detaljer utgör "anteckningarna" för övningen och BÖR inkludera:
        *   Förslag på antal set och repetitioner (för styrkeövningar), eller distans/tid/kalorier (för konditionsövningar). Exempel: "3 set x 8-12 reps", "5 set x 500m rodd", "10 minuter AMRAP", "3 set x 45 sekunder".
...
*   Roddmaskin: 5 set x 500m, vila 60 sek. Håll ett jämnt tempo.
    ...
    for (const line of lines) {
        ...
        if (currentBlock) {
            ...
            if (exerciseMatch && exerciseMatch[1]) {
                ...
                const delimiters = [": "]; // Stricter delimiter based on prompt
                ...
                if (!splitSuccessful) {
                     // Fallback for " - " or other patterns if strict ":" is not found
                ...
                if (!splitSuccessful) { // If still no split, use regex-based fallback
                    const notesPatternRegex = /(\d+\s*set|\d+-\d+\s*rep|\bAMRAP\b|\d+\s*min|vila\s*\d+|\d+m\b|\d+sek\b|\d+kcal\b)/i;
