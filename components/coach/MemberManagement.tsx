...
export const MemberManagement: React.FC<MemberManagementProps> = ({ participants, setParticipants, allParticipantGoals, setParticipantGoals, allActivityLogs, coachNotes, setCoachNotes, locations, memberships, staffMembers, loggedInStaff, ai, oneOnOneSessions, setOneOnOneSessions, setGoalCompletionLogs, workouts, setWorkouts, workoutCategories, staffAvailability }) => {
  ...
  const handleConfirmBulkDelete = () => {
    setParticipants(prev => prev.filter(p => !selectedMemberIds.includes(p.id)));
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredAndSortedParticipants.map(p => {
                            ...
                            return (
                                <tr key={p.id} className={`${rowClass} ${isSelected ? 'bg-flexibel/20' : ''}`}>
                                    {loggedInStaff?.role === 'Admin' && (
                                        <td className="p-4">
                                            <input
                                                type="checkbox"
