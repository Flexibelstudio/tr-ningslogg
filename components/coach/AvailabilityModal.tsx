...
export const AvailabilityModal: React.FC<AvailabilityModalProps> = ({ isOpen, onClose, onSave, onDelete, staffMember, availabilityToEdit, initialTime }) => {
  ...
  const handleSave = () => {
    ...
    // For recurring events, we need a reference date to create the time, but the date itself isn't stored.
    // Use local date to avoid timezone issues where creating an event after midnight local time but before midnight UTC
    // would anchor the event to the previous day.
    ...
    // Create local Date objects based on user input to ensure time is correct in user's timezone.
    const startLocal = new Date(year, month - 1, day, startH, startM);
    const endLocal = new Date(year, month - 1, day, endH, endM);

    const saveData: StaffAvailability = {
        id: availabilityToEdit?.id || crypto.randomUUID(),
        staffId: staffMember.id,
        // Convert local dates to UTC ISO strings for storage.
        ...
        recurringDetails: isRecurring ? {
            ...
            // Ensure end date includes the full day by interpreting it as local time, not UTC midnight.
            recurringEndDate: recurringEndDate ? new Date(`${recurringEndDate}T23:59:59`).toISOString() : undefined,
        } : undefined,
    };
    onSave(saveData);
  };
