...
export const ClientJourneyView: React.FC<ClientJourneyViewProps> = ({
  ...
  const { onboardingCount, checkinCount, expiringCount } = useMemo(() => {
    ...
    for (const p of participants) {
        if ((!p.isActive && !p.isProspect) || (!p.startDate && !p.creationDate)) continue;
        
        const referenceDateString = p.startDate || p.creationDate;
        if (!referenceDateString) continue;

        const startDate = new Date(referenceDateString);
  ...
  const journeyData = useMemo<ClientJourneyEntry[]>(() => {
    return participants
      .filter(p => (p.isActive && !p.isProspect && p.startDate) || (p.isProspect && p.creationDate))
      .map(p => {
        const today = new Date();
        const referenceDate = new Date(p.isProspect ? p.creationDate! : p.startDate!);
        ...
        // 2. Onboarding
        else if (ptPassCount < 4 && daysSinceStart < 45) {
          ...
          if (p.isProspect && ptPassCount === 0 && myScheduledPTSessions.length === 0) {
              nextActionTextForOnboarding = 'Boka introsamtal/första PT-pass';
              nextActionPriorityForOnboarding = 'high';
          } else if (myScheduledPTSessions.length > 0) {
        ...
        // 3. Post-Onboarding Journey
          ...
          const hasCheckInAroundDays = (day: number, range: number) => {
             const targetDate = new Date(referenceDate.getTime() + day * 24 * 60 * 60 * 1000);
                    ...
                    {filteredParticipants.map(p => {
                        ...
                        return (
                            <tr key={p.id}>
                                ...
                                <td className="px-4 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${priorityClasses[p.nextActionPriority]}`}>
                                        {p.nextActionText}
                                    </span>
                                </td>
