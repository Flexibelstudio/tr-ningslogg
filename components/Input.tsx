...
export const Input: React.FC<InputProps> = ({ label, id, error, className, inputSize = 'md', containerClassName, ...props }) => {
  ...
  if (inputSize === 'sm') {
    ...
    sizeSpecificStyle = 'px-5 py-3 text-2xl rounded-xl';
    labelStyle = 'block text-2xl font-medium text-gray-700 mb-1.5';
  } else { // md (default)
    sizeSpecificStyle = 'px-4 py-2.5 text-xl rounded-lg';
  }
...
export const Select: React.FC<SelectProps> = ({ label, id, error, className, options, inputSize = 'md', containerClassName, ...props }) => {
  ...
  if (inputSize === 'sm') {
    ...
    sizeSpecificStyle = 'pl-5 pr-12 py-3 text-2xl rounded-xl';
    labelStyle = 'block text-2xl font-medium text-gray-700 mb-1.5';
  } else { // md (default)
    sizeSpecificStyle = 'pl-4 pr-10 py-2.5 text-xl rounded-lg';
  }
