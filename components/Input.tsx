...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({ label, id, error, className, inputSize = 'md', containerClassName, ...props }, ref) => {
  ...
  const baseStyle = 'block w-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-flexibel/40 focus:border-flexibel text-gray-900 placeholder-gray-500 bg-gray-50 focus:bg-white';
  
  let sizeSpecificStyle = '';
  let labelStyle = 'block text-sm font-medium text-gray-600 mb-1';

  if (inputSize === 'sm') {
    sizeSpecificStyle = 'px-2 py-1.5 text-base rounded-md'; // Changed from text-sm to prevent iOS zoom
    labelStyle = 'block text-sm font-medium text-gray-600 mb-0.5';
  } else if (inputSize === 'lg') {
    sizeSpecificStyle = 'px-4 py-2.5 text-lg rounded-xl';
    labelStyle = 'block text-lg font-medium text-gray-600 mb-1.5';
  } else { // md (default)
    sizeSpecificStyle = 'px-3 py-2 text-base rounded-xl';
  }
      ...
      {isPassword ? (
        <div className="relative">
          ...
          <button
            ...
            className="absolute inset-y-0 right-0 flex items-center p-3 text-gray-500 active:text-gray-700 focus:outline-none focus:ring-2 focus:ring-flexibel/40 rounded-r-xl"
...
export const Select: React.FC<SelectProps> = ({ label, id, error, className, options, inputSize = 'md', containerClassName, ...props }) => {
  const baseStyle = 'block w-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-flexibel/40 focus:border-flexibel text-gray-900 bg-gray-50 focus:bg-white';
  ...
  if (inputSize === 'sm') {
    ...
    sizeSpecificStyle = 'pl-4 pr-10 py-2.5 text-lg rounded-xl';
    labelStyle = 'block text-lg font-medium text-gray-600 mb-1.5';
  } else { // md (default)
    sizeSpecificStyle = 'pl-3 pr-10 py-2 text-base rounded-xl';
  }
