...
export const Textarea: React.FC<TextareaProps> = ({ label, id, error, className, ...props }) => {
  const baseStyle = 'block w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-flexibel/40 focus:border-flexibel sm:text-xl text-xl text-gray-900 placeholder-gray-500'; 
  const errorStyle = error ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : '';
  const effectiveId = id || props.name;

  return (
    <div className="w-full">
      {label && <label htmlFor={effectiveId} className="block text-xl font-medium text-gray-700 mb-1">{label}</label>} 
