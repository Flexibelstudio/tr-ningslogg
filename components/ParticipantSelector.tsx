...
export const ParticipantSelector: React.FC<ParticipantSelectorProps> = ({ participants, onSelectParticipant, onGoBack }) => {
  const activeParticipants = participants.filter(p => {
    ...
    if (p.endDate) {
      ...
      const endDate = new Date(p.endDate); // endDate is YYYY-MM-DD
      
      // The user's access expires *after* their end date.
      // So if endDate is today, they should still be able to log in.
      if (endDate < today) {
        return false; // endDate has passed
      }
    }
    return true; // Active and endDate is not in the past
  });
