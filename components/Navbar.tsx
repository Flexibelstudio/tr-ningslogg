...
const GoalIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19.5 10.5l-7.5 7.5-3.5-3.5" />
  </svg>
);
const FlowIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const { user, logout, isImpersonating, stopImpersonating, isStaffViewingAsParticipant, viewAsParticipant, stopViewingAsParticipant, organizationId, currentParticipantId, currentRole } =
    useAuth();
  ...
  const destinationView = isStaffViewingAsParticipant ? `Till ${user.roles.systemOwner ? 'Systemägar-vy' : (loggedInStaff?.role + '-vy') || 'Admin-vy'}` : 'Till Medlemsvy';

  const MenuItem: React.FC<{ onClick: () => void; children: React.ReactNode }> = ({ onClick, children }) => (
    <button
      onClick={onClick}
      className="w-full text-left px-4 py-2 text-base text-gray-700 active:bg-gray-100 active:text-gray-900 flex items-center gap-3 transition-colors"
      role="menuitem"
    >
      {children}
    </button>
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          {currentRole === UserRole.PARTICIPANT && (
            <h1 className="text-2xl font-bold text-gray-800 hidden md:block">{currentUserProfile?.name ? `, ${currentUserProfile.name.split(' ')[0]}!` : ''}</h1>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {currentRole === UserRole.PARTICIPANT && (
            <div className="flex items-center gap-1 sm:gap-2">
              <button
                ...
                className="p-2 rounded-full text-gray-500 hover:text-flexibel active:text-flexibel active:bg-flexibel/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flexibel active:scale-95 transition-transform"
                title="Mål & Plan"
                aria-label="Mål & Plan"
              >
                <GoalIcon />
              ...
              <button
                ...
                className="relative p-2 rounded-full text-gray-500 hover:text-flexibel active:text-flexibel active:bg-flexibel/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flexibel active:scale-95 transition-transform"
                title="Flöde"
                aria-label="Flöde"
              >
                <FlowIcon />
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="relative rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-flexibel transition-transform duration-200 active:scale-105"
              aria-haspopup="true"
              aria-expanded={isMenuOpen}
              aria-label="Öppna användarmeny"
            >
              <Avatar name={currentUserProfile?.name || user.name} photoURL={currentUserProfile?.photoURL} className="h-9 w-9" />
              {hasUnreadUpdate && <span className="absolute top-0 right-0 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white" />}
            </button>
            {isMenuOpen && (
              <div
                ...
                <div className="py-1">
                  <div className="px-4 py-3 border-b">
                    ...
                    <p className="text-xs text-gray-500">
                      {currentRole === UserRole.PARTICIPANT ? 'Medlem' : user.roles.systemOwner ? (isImpersonating ? 'Adminvy' : 'Systemägare') : loggedInStaff?.role || 'Personal'}
                    </p>
                  </div>
                  ...
                  >
                    ...
                    {hasUnreadUpdate && <span className="block h-2 w-2 rounded-full bg-red-500" />}
                  </button>
