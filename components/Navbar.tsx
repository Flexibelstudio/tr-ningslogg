...
import { UserRole, ParticipantProfile } from '../types';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // --- derive current participant profile/name ---
  const currentUserProfile = useMemo<ParticipantProfile | null>(() => {
    if (currentRole === UserRole.PARTICIPANT) {
      return participantDirectory.find((p) => p.id === currentParticipantId) || null;
    }
    if (user?.linkedParticipantProfileId) {
      return participantDirectory.find((p) => p.id === user.linkedParticipantProfileId) || null;
    }
    return null;
  ...
  const handleSwitchView = () => {
    if (isStaffViewingAsParticipant) stopViewingAsParticipant();
    else viewAsParticipant();
    setIsMenuOpen(false);
  };
  ...
  const handleOpenLatestUpdateLocal = () => {
    onOpenLatestUpdate();
    setIsMenuOpen(false);
  };
    ...
    <nav className="bg-white shadow-lg border-b border-gray-200 sticky top-0 z-40">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {branding?.logoBase64 ? (
            <img
              ...
              alt={`${currentOrganization?.name || APP_NAME} logotyp`}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {/* Enable Push in topbar for ≥sm so it doesn't overlap avatar */}
          {currentRole === UserRole.PARTICIPANT && (
            ...
            {isMenuOpen && (
              ...
              >
                <div className="py-1">
                  ...
                  {/* enable push also inside menu (for mobile) */}
                  {currentRole === UserRole.PARTICIPANT && (
                  ...
                  {/* latest update */}
                  <button
                    onClick={handleOpenLatestUpdateLocal}
                    className="w-full text-left px-4 py-2 text-base text-gray-700 hover:bg-gray-100 hover:text-gray-900 flex items-center gap-3 transition-colors"
                    role="menuitem"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-500" fill="none"
