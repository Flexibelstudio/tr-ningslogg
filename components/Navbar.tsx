...
import { ParticipantNavbarActions } from './participant/ParticipantArea';
...
interface NavbarProps {
  onOpenProfile: () => void;
  participantActions: ParticipantNavbarActions | null;
}

export const Navbar: React.FC<NavbarProps> = ({ onOpenProfile, participantActions }) => {
        ...
        <div className="flex items-center gap-2">
            {participantActions && (
                <div className="flex items-center">
                    <Button onClick={participantActions.onOpenGoalModal} variant='ghost' size="sm" className="h-auto p-2 text-flexibel" title="Mål & Plan" aria-label="Mål & Plan">
                        <GoalIcon />
                    </Button>
                    {participantActions.aiRecept && (
                        <Button onClick={participantActions.onOpenAiRecept} variant='ghost' size="sm" className="h-auto p-2 text-flexibel" title="Coachens tips för att du ska nå ditt mål" aria-label="Coachens tips för att du ska nå ditt mål">
                            <AiReceptIcon />
                        </Button>
                    )}
                    <Button onClick={participantActions.onOpenFlowModal} variant='ghost' size="sm" className="relative h-auto p-2 text-flexibel" title="Flöde" aria-label="Flöde">
                        <FlowIcon />
                        {participantActions.newFlowItemsCount > 0 && (
                            <span className="absolute top-0 right-0 h-4 w-4 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                                {participantActions.newFlowItemsCount}
                            </span>
                        )}
                    </Button>
                    <Button onClick={participantActions.onOpenCommunity} variant='ghost' size="sm" className="relative h-auto p-2 text-flexibel" title="Community" aria-label="Community">
                        <CommunityIcon />
                        {participantActions.pendingRequestsCount > 0 && (
                            <span className="absolute top-0 right-0 h-4 w-4 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                            {participantActions.pendingRequestsCount}
                            </span>
                        )}
                    </Button>
                </div>
