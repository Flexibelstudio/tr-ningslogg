...
const WorkoutCard: React.FC<{
  ...
  const intensityDetail = workout.category === 'PT-bas' && workout.intensityLevel ? INTENSITY_LEVELS.find((l) => l.value === workout.intensityLevel) : null;
  const isRestricted = workout.isRestricted;

  return (
    <div
      key={workout.id}
      className={`relative p-4 rounded-lg shadow-md transition-all duration-200 ease-out border ${
        isRestricted ? 'bg-gray-100 border-gray-300' : 'bg-white border-gray-200 hover:shadow-lg'
      }`}
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-3 relative z-10">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
            ...
            {intensityDetail && (
              <span className={`inline-block ${intensityDetail.twBadgeClass} text-xs font-semibold px-2 py-0.5 rounded-full`}>Fokus: {intensityDetail.label}</span>
            )}
        ...
        {isRestricted ? (
          ...
          <Button onClick={() => onStart(workout)} size="md" className="w-full sm:w-auto flex-shrink-0 mt-2 sm:mt-0" aria-label={`Starta passet ${workout.title}`}>
            Starta Pass
      ...
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 -mr-2">
        ...
        {!hasPersonalWorkouts && !hasGeneralWorkouts && (
          <div className="text-center py-10">
            ...
            <p className="mt-1 text-base text-gray-500">Inga {categoryFilter ? `${categoryFilter.toLowerCase()} ` : ''}pass är tillgängliga för loggning just nu.</p>
          </div>
        )}
