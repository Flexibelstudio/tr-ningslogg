...
import { WeightComparisonItem, GenderOption, StrengthStandard, StrengthLevel, LiftType, AllUserProvidedStrengthMultipliers, StrengthStandardDetail, UserProvidedLiftMultipliers, WorkoutCategory } from './types'; // Updated import type from AnimalWeight

export const FLEXIBEL_PRIMARY_COLOR = '#51A1A1'; // Updated color
export const APP_NAME = 'Flexibel Träningslogg';
...
export const LOCAL_STORAGE_KEYS = {
  ...
  PARTICIPANT_MENTAL_WELLBEING: 'flexibel_participantMentalWellbeing', // New key
  WELCOME_MESSAGE_SHOWN_PARTICIPANT: 'flexibel_welcomeMessageShown_participant', 
  LAST_FEEDBACK_PROMPT_TIME: 'flexibel_lastFeedbackPromptTime', // New key for feedback prompt cooldown
};
...
ALL_LIFT_TYPES.forEach(liftType => {
  let liftKeyLookup = liftType.toLowerCase();
  ...
  if (validMultiplierKeys.includes(liftKeyLookup as keyof AllUserProvidedStrengthMultipliers)) {
    const multipliersForThisLift = USER_PROVIDED_STRENGTH_MULTIPLIERS[liftKeyLookup as keyof AllUserProvidedStrengthMultipliers];
