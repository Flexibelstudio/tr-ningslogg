...
const AppContent: React.FC = () => {
    ...
    const handleToggleCommentReaction = useCallback((logId: string, logType: 'workout' | 'general' | 'coach_event' | 'one_on_one_session', commentId: string) => {
        if (!auth.currentParticipantId) return;
        const participantId = auth.currentParticipantId;
    
        const updater = <T extends { id: string; comments?: Comment[] }>(prevLogs: T[]): T[] => {
            return prevLogs.map(log => {
                if (log.id === logId) {
                    const updatedComments = (log.comments || []).map(comment => {
                        if (comment.id === commentId) {
                            ...
                            if (myReactionIndex > -1) { // User has liked, so unlike.
        ...
        switch(logType) {
            ...
            case 'one_on_one_session':
                setOneOnOneSessionsData(updater as any);
                break;
        }
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-gray-100 bg-dotted-pattern bg-dotted-size z-50">
                    <div>
                        {logo ? (
                           ...
                           <div className="text-5xl font-bold" style={{ color: '#3bab5a' }}>
                                Träningslogg
                           </div>
                        )}
                    </div>
