    ...
    useEffect(() => {
        ...
        const fetchGlobals = async () => {
            ...
            try {
                ...
                if (user) {
                    // System Owners are allowed to read the entire 'users' collection.
                    if (user.roles.systemOwner) {
                        const users = await firebaseService.get('users');
                        setAllUsers(users);
                    } else {
                        // Other logged-in users cannot list all users due to security rules.
                        // For now, we will fall back to mock data. A more advanced implementation
                        // might fetch users within their own organization if rules permitted.
                        console.warn("Logged in as non-System-Owner. Cannot list all users. Falling back to mock data for users.");
