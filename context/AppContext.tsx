        ...
        if (organizationId) {
            ...
            const loadAllDataFromFirestore = async () => {
                try {
                    ...
                    setLeaderboardSettings(data.leaderboardSettings || { leaderboardsEnabled: false, weeklyPBChallengeEnabled: false, weeklySessionChallengeEnabled: false });
                    setCoachEvents(data.coachEvents || []);
                    setConnections(data.connections || []);
                    setLastFlowViewTimestamp(data.lastFlowViewTimestamp || null);
                    setLocations(data.locations || []);
                    setStaffMembers(data.staffMembers || []);
                    setMemberships(data.memberships || []);
                    setWeeklyHighlightSettings(data.weeklyHighlightSettings || { isEnabled: false, dayOfWeek: 1, time: '09:00', studioTarget: 'separate' });
                    ...
                    setIntegrationSettings(data.integrationSettings || { enableQRCodeScanning: false, isBookingEnabled: false, isClientJourneyEnabled: true, isScheduleEnabled: true });
